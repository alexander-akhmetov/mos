...
pub struct VirtualFS {
    ...
    mountpoints: BTreeMap<String, Box<dyn FileSystem>>,
}
...
impl VirtualFS {
    ...
    pub fn mount(&mut self, mountpoint: &str, fs: Box<dyn FileSystem>) {
        self.mountpoints.insert(utils::normalize(mountpoint), fs);
        system_log!("VFS: attached new FS at {}", mountpoint);
    }

    pub fn get_file(&self, path: &str) -> Option<Box<dyn FileDescriptor>> {
    ...
    fn get_fs(&self, path: &str) -> (Option<&Box<dyn FileSystem>>, String) {
    ...
    pub fn list_dir(&self, path: &str) -> Option<Vec<Box<dyn FileDescriptor>>> {
